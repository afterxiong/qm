...
public class CmdDELE extends FtpCmd implements Runnable {
	...
	protected  int ftpClientVersion;
	
	public CmdDELE(SessionThread sessionThread, String input) {
		super(sessionThread, CmdDELE.class.toString());
		this.input = input;
		this.ftpClientVersion = sessionThread.getFtpClientVision();
	}
	
	public void run(){
		myLog.l(Log.INFO, "DELE executing");
		if (ftpClientVersion != 0){
			//因删除由server端处理,所以注释掉Delete命令
			sessionThread.writeString("250 File not need delete in Client for version 1\r\n");
		}else{
