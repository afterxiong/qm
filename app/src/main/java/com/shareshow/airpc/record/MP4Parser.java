...
class MP4Parser {
    ...
    public StsdBox getStsdBox() throws IOException {
        try {
            return new StsdBox(file,
                    getBoxPos("/moov/trak/mdia/minf/stbl/stsd"));
        } catch (IOException e) {
    ...
    private void parse(String path, long len) throws IOException {
        ...
        while (sum < len) {
            ...
            if (validBoxName()) {
                ...
                newlen = (buffer[3] & 0xFF | (buffer[2] & 0xFF) << 8
                        | (buffer[1] & 0xFF) << 16 | (buffer[0] & 0xFF) << 24) - 8;
    ...
    private boolean validBoxName() {
        for (int i = 0; i < 4; i++) {
            if ((buffer[i + 4] < 97 || buffer[i + 4] > 122)
...
class StsdBox {
    ...
    private boolean findSPSandPPS() {
         ...
         * AVCLevelIndication; bit(6) reserved = �??11111’b; unsigned int(2)
         * lengthSizeMinusOne; bit(3) reserved = �??11’b; unsigned int(5)
        ...
        try {
            ...
            StringBuffer spsBuffer=new StringBuffer();
            for (int i = 0; i < sps.length; i++) {
                spsBuffer.append(sps[i]+" ");
            }
            Log.e("xieronghua", "sps "+spsBuffer.toString());
            // Here we extract the PPS parameter
            fis.skipBytes(2);
            ppsLength = 0xFF & fis.readByte();
            pps = new byte[ppsLength];
            fis.read(pps, 0, ppsLength);
            StringBuffer ppsBuffer=new StringBuffer();
            for (int i = 0; i < pps.length; i++) {
                ppsBuffer.append(pps[i]+" ");
            }
            Log.e("xieronghua", "pps "+ppsBuffer.toString());
