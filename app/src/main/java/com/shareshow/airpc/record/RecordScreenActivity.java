...
/**
 * 截屏功能的类 从5.0系统开始； startrecord是请求截屏的方法
 *
 * @author tanwei
 */
public class RecordScreenActivity extends AppCompatActivity implements INotify,EncoderListener,ITimerCallback {
  ...
  //public DataTrans datatrans = DataTrans.getInstance();// 手机允许截屏就用该类去调用 投屏底层提供的方法
    ...
    //  private AudioTranAPI audioTranAPI = new AudioTranAPI(this);
    ...
    public void startrecord(ConnectFTPListener mLoinListener){
...
//        VIDEO_HEIGHT = dm.heightPixels;
            if (android.os.Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP){
                mMediaProjectionManager = (android.media.projection.MediaProjectionManager) getSystemService(Context.MEDIA_PROJECTION_SERVICE);
                                ...
                                if (mMuxer != null){
                                    ...
                                    if (temp_file.exists()){
                                        try{
                                            ...
                                            Log.e(TAG,
                                                    "parse sps faild for file not found");
                                        }catch (IOException e){
...
//                             pps.getBytes(), outData, mVideoBufferInfo.size);
                            ...
                            if(isFirstData){
                                ...
                                iFrameforceTimer.Start(500, RecordScreenActivity.this);
                                iForceCnt = 0;
                            }
...
//                    VIDEO_HEIGHT =(typeT ==0 ? width:hight);
                ...
                Toast.makeText(this,"帧率："+frameRate+" 码率："+bitRate+"---"+rate+" i帧间隔："+i_frame_interval+" 其他："+repeat_frame+"  分辨率："+VIDEO_WIDTH+"*"+VIDEO_HEIGHT+"  "
